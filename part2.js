...
let inputArray = [
    ...
    "rsznrsrtcz2threerk5ggdrtfmktrjjrdpjvvhs,hhqszzmbzp",
    "nine53v",
    "6khbvone",
    "tvzoneight5khzglntxjgb",
    "qtgx9zhtrqcnlblvrsngsfkeightninefvlnpjb,xsix",
    "tljmkjbb1kncvqsix9jthkqlkvkztjjbtrbqjxj,mrblzzjfive",
    "eight6l",
    ...
    "8pztdljxbjjthreenineeightseven7crkdr8ei,ghtwocb",
    ...
    "fournpgkxtp4twojtmqgjgnzdxrnvdxfsltzpfl,mg1nine",
    "one2one3",
    "twosixfgklbnprck26vstrjkfxgmbhhceightwo,mz",
    "bxxbt5two2g7",
    "5gnineone8697five",
    ...
    "qlkbjplh1",
    "5sevenfivefour",
    "2ninejsjfngfhhldkkghqlmflqfbqpvtnsthgj8,four",
    ...
    "gqxqtdktrltmhqp4fivelbqsrklgfllrh48fxgd,lcvsrc",
...
inputArray.forEach((str) => {
    const result = findDigitsInText(str);
    newInputArray.push(result.join(''));
});

function findDigitsInText(data) {
    const digitWords = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
    const foundDigits = [];
    for (let i = 0; i < data.length; i++) {
        if (!isNaN(data[i])) {
            foundDigits.push(data[i]);
        }
        for (let j = 0; j < digitWords.length; j++) {
            const digitWord = digitWords[j];

            if (data.substring(i, i + digitWord.length).toLowerCase() === digitWord) {
                foundDigits.push(j);
                i += digitWord.length - 2;
                break;
            }
        }
    }
...
function findFirstAndLastDigit(data) {
    for (let i = 0; i < data.length; i++) {    
        ...
        for (let j = 0; j < inputString.length; j++) {
            if (!isNaN(inputString[j])) {
                ...
                firstDigit = parseInt(inputString[j]);
                }
                lastDigit = parseInt(inputString[j]);
            }
        }
        stringifiedNumber = `${firstDigit}${lastDigit}`;
        sumArray.push(stringifiedNumber);
...
function sumNumbers(sumArray) {
    for (let i = 0; i < sumArray.length; i++) {
        let number = parseInt(sumArray[i]);
        sum += number;
    }
    return sum;
}
...
findFirstAndLastDigit(newInputArray);
sumNumbers(sumArray);
